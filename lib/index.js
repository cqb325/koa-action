...
module.exports = class KoaAction {
    ...
    loadMiddlewares(){
        ...
        if (config.middleware && config.middleware.indexOf('database') !== -1) {
    ...
    handRouter(method, pathname, handler){
        if (config.middleware && config.middleware.indexOf('koa-body') !== -1) {
            ...
            this.router[method](pathname, async (ctx, next)=>{
                let kaCtx = new Context(ctx, this.urlMap);
                kaCtx.bindServices(this.services);
                await handler.apply(kaCtx,[]);
            });
