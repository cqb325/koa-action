...
class Captcha {
    constructor(options) {
        ...
        options.fontPath = options.fontPath || node_path_1.default.join(__dirname, './font/captchaFont.ttf');
        this.options = options;
    }
    async generate() {
        const canvas = (0, canvas_1.createCanvas)(this.options.canvasWidth, this.options.canvasHeight);
        ...
        for (i = 0; i < text.length; i++) {
            const transform = new canvas_1.DOMMatrix([Math.random() * 0.5 + 1, Math.random() * 0.4,
                Math.random() * 0.4,
                Math.random() * 0.2 + 1,
